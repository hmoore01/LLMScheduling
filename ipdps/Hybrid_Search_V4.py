...
def hybrid_search_p(lut_list, func_distribution_arr, leftover_resource_time_arr, req_distribution_arr, epoch, original_req_distribution_arr, step_size=0.1, number_of_random_search=20):
    brain = GradientBoostingRegressor()
    total_number_of_func_type = func_distribution_arr.shape[0]
    fit_frequency = number_of_random_search
    ## loading weights
    number_of_population = 20
    number_of_objective = 4
    number_of_worker = 4
    ...
    for search_epoch in range(100):
        ...
        ## local search part
        if search_epoch < number_of_random_search:
            search_location_list = random.sample(list(range(20)), number_of_worker)
            if search_epoch == (number_of_random_search - fit_frequency):
                design_trajectory = []
                best_fit_trajectory = []
        else:
            if search_epoch == number_of_random_search:
                brain.fit(design_trajectory, best_fit_trajectory)
                design_trajectory = []
                best_fit_trajectory = []
            elif (search_epoch-number_of_random_search) % fit_frequency == 0:
                brain.fit(design_trajectory, best_fit_trajectory)
                design_trajectory = []
                best_fit_trajectory = []
            if random.random() < 0.9:
                search_location_list, prediction = smart_brain_decision(weight_group, value_arr, brain)
            else:
                search_location_list = random.sample(list(range(number_of_population)), k = number_of_objective)
        ...
        for result,search_location in zip(results,search_location_list):
            weight = weight_arr[search_location]
            fit_old = local_fit(value_arr[search_location], weight)
            fit_new = local_fit(result[0], weight)
            fit_difference = (fit_old - fit_new)*100
            if fit_difference < 0:
                print('warning')
                print(value_arr[search_location])
                print(result[0])
                print(fit_old, fit_new, fit_difference)
                exit()
            design_trajectory.append(copy.deepcopy(value_arr[search_location]))
            best_fit_trajectory.append(fit_difference)
        ...
        design_arr, value_arr = update_population(value_arr, design_arr, all_children_value_list, all_children_design_list, weight_arr)
        # print('GA out')    
    return design_arr, value_arr, standard_objective  ##should return pareto front

def smart_brain_decision(weight_group, value_arr, brain):
    chosen_starting_points = []
    prediction = []
    for i in range(len(weight_group)):
        current_pareto_front = []
        for index in weight_group[i]:
            design = value_arr[index]
            current_pareto_front.append(design)
            
        pred = brain.predict(current_pareto_front)
        pre = pred.tolist()
        sorted_pre = sorted(range(len(pre)), key=lambda i: pre[i], reverse = True)
        index = sorted_pre[0]
        prediction.append(pre[index])
        chosen_starting_points.append(weight_group[i][index])
    return chosen_starting_points, prediction
    
def update_population(value_arr, design_arr, all_children_value_list, all_children_design_list, weight_arr):
    update_count = 0
...
def perturb_and_simulate_10time_p(args):
    search_location, value_arr, design_arr, weight_arr, step_size, lut_list, func_distribution_arr, leftover_resource_time_arr, original_req_distribution_arr, req_distribution_arr, epoch = args
...
def perturb_and_simulate_10time(search_location, value_arr, design_arr, weight_arr,
    ...
    for _ in range(5):
        ...
        if local_fit(search_objectives, weight_arr[search_location]) < local_fit(the_value, weight_arr[search_location]):
