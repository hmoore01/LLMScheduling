...
def hybrid_search_p(lut_list, func_distribution_arr, leftover_resource_time_arr, req_distribution_arr, epoch, original_req_distribution_arr, step_size=0.1, number_of_random_search=20):
    ...
    search_epoch = 0
    while time.time()-search_epoch_time < 60*15:
    # while time.time()-search_epoch_time < 10:
    # for search_epoch in range(300):
        ...
        if search_epoch < number_of_random_search:
            ...
            if random.random() < 0.9:
                # search_location_list, prediction = smart_brain_decision(weight_group, design_arr, brain)
        ...
        for result,search_location in zip(results,search_location_list):
            ...
            fit_old = local_fit(value_arr[search_location], weight, scaler)
            fit_new = local_fit(result[0], weight, scaler)
            fit_difference = (fit_old - fit_new)*1
            if fit_difference < 0:
                print('warning')
                ...
                exit()
            # design_trajectory.append(copy.deepcopy(design_arr[search_location][0,:].flatten().tolist()))
...
def smart_brain_decision(weight_group, design_arr, brain):
    chosen_starting_points = []
    prediction = []
    for i in range(len(weight_group)):
        current_pareto_front = []
        for index in weight_group[i]:
            # design = design_arr[index][0,:].flatten().tolist()
            design = design_arr[index]
