...
def copy_score_policy(lut_list, func_distribution_arr, leftover_resource_time_arr, vm_distribution_arr):
    global first_interval_flag
    global this_interval_avail_resource_time
    global next_interval_avail_resource_time
    global number_of_node
...
def copy_multi_score_policy(lut_list, func_distribution_arr, leftover_resource_time_arr, vm_distribution_arr):
    global first_interval_flag
    global this_interval_avail_resource_time
    global next_interval_avail_resource_time
    global number_of_node
    ...
    for i in range(len(scored_prewarm_func_list)):
        ...
        for j in range(number_of_container):
            ...
            new_vm_distribution_arr[func_id,the_node_id+1] = 1
            new_vm_distribution_arr[func_id,0]+=1
            cumulative_resource_time_arr[the_node_id,:epoch_length]+=idle_res_usage
            index_count+=1
            index_count%=number_of_node
...
def copy_dual_optimizer(lut_list, func_distribution_arr, leftover_resource_time_arr, vm_distribution_arr, epoch):
    total_number_of_func_type = func_distribution_arr.shape[0]
    new_vm_distribution_arr = copy.deepcopy(vm_distribution_arr)
    # new_vm_distribution_arr[:,:]=0
    search_leftover_resource_time_arr=copy.deepcopy(leftover_resource_time_arr)
    ...
    start_time = time.time()
    # cumulative_resource_time_arr, total_violation, total_invocation, energy_cost, carbon, new_vm_distribution_arr = multi_step_slo_optimizer(
    #     lut_list, func_distribution_arr, leftover_resource_time_arr, vm_distribution_arr, new_vm_distribution_arr, epoch, step=100)
    while (time.time() - start_time) < 180:
        if total_violation/total_invocation < 0.05:
            cumulative_resource_time_arr, total_violation, total_invocation, energy_cost, carbon, new_vm_distribution_arr = single_step_carbon_optimizer(
                lut_list, func_distribution_arr, leftover_resource_time_arr, vm_distribution_arr, new_vm_distribution_arr, epoch, step=1)
        else:
            cumulative_resource_time_arr, total_violation, total_invocation, energy_cost, carbon, new_vm_distribution_arr = single_step_slo_optimizer(
                lut_list, func_distribution_arr, leftover_resource_time_arr, vm_distribution_arr, new_vm_distribution_arr, epoch, step=1)
    
    if total_violation/total_invocation > 0.05:
        cumulative_resource_time_arr, total_violation, total_invocation, energy_cost, carbon, new_vm_distribution_arr = single_step_carbon_optimizer(
            lut_list, func_distribution_arr, leftover_resource_time_arr, vm_distribution_arr, new_vm_distribution_arr, epoch, step=1)      
    # print(epoch, energy_cost, total_violation/total_invocation, carbon)
    # leftover_resource_time_arr = update_leftover(new_vm_distribution_arr, vm_distribution_arr, scored_prewarm_func_list, leftover_resource_time_arr)
    # cumulative_resource_time_arr, total_violation, total_invocation, vm_distribution_arr, slo_violation_arr = simulation(
    #     vm_distribution_arr, new_vm_distribution_arr, func_distribution_arr, leftover_resource_time_arr, func_priority_list, lut_list)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f', '--framework', type=str, help='framework', default='DSLO', choices= ['SLO', 'Load', 'Ideal', 'Back', 'Hybrid', 'Score', 'Binary', 'Mscore', 'DSLO', 'Qtrain', 'Qtest'])
    ...
    for i in range(0,number_of_epoch): 
        ...
        objective_arr[i,:] = total_energy_cost, violation_count, invocation_count, carbon, system_load
        # print(i, time.time() - s_time)
        # print(i, objective_arr[i,:], np.count_nonzero(vm_distribution_arr[:,0], axis=0), sum(np.count_nonzero(vm_distribution_arr[:,1:], axis=0)), time.time() - s_time)
        print(i, total_energy_cost, violation_count/invocation_count, carbon, system_load, np.count_nonzero(vm_distribution_arr[:,0], axis=0), sum(np.count_nonzero(vm_distribution_arr[:,1:], axis=0)), time.time() - s_time)

    print("total_energy_cost, violation_rate, carbon, system_load")
    print(np.sum(objective_arr[:,0]), np.sum(objective_arr[:,1])/np.sum(objective_arr[:,2]), np.sum(objective_arr[:,3]), np.mean(objective_arr[:i,4])) #total_energy_cost, violation_count, carbon, system_load
